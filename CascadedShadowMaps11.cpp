...
void InitApp()
{
    ...
    g_HUD.AddCheckBox( IDC_BLEND_BETWEEN_MAPS_CHECK, desc, 0, iY+15, 170, 23, false );
    g_HUD.AddSlider( IDC_BLEND_MAPS_SLIDER, 40, iY+33, 100, 15, 0, 500, ( INT )
        10.0f );
    //UV级别blend区间
    g_CascadedShadow.m_fBlurBetweenCascadesAmount = g_HUD.GetSlider( IDC_BLEND_MAPS_SLIDER )->GetValue()*0.001f;
...
void CALLBACK OnGUIEvent( UINT nEvent, INT nControlID, CDXUTControl* pControl, void* pUserContext )
{
    switch( nControlID )
    {
        ...
        case IDC_BLEND_MAPS_SLIDER : 
        {
            ...
            g_CascadedShadow.m_fBlurBetweenCascadesAmount = (float)val * 0.001f;
            WCHAR dta[256];
...
HRESULT CreateD3DComponents( ID3D11Device* pd3dDevice ) 
{
    ...
    D3DXVECTOR3 vecAt( -600.0f,0.0f,-600.0f );
