...
struct VS_OUTPUT
{
    float3 vNormal                          : NORMAL;
    float2 vTexcoord                        : TEXCOORD0;
    float4 vTexInShadowView					    : TEXCOORD1;
    float4 vPosition                        : SV_POSITION;
    float4 vInterpPos                       : TEXCOORD2; 
    float  vDepthInWorldView                           : TEXCOORD3;
};
...
VS_OUTPUT VSMain( VS_INPUT Input )
{
    ...
    Output.vDepthInWorldView = mul( Input.vPosition, m_mWorldView ).z ; 
       
    // Transform the shadow texture coordinates for all the cascades.
    Output.vTexInShadowView = mul( Input.vPosition, m_mShadowView );
    return Output;
...
{
	vShadowUV_InTexCoord = vPosInShadowView * m_vCascadeScale[iCascadeIndex];
	vShadowUV_InTexCoord += m_vCascadeOffset[iCascadeIndex];
}
...
void TransformLogicU_ToNativeU(in int iCascadeIndex, in out float4 vShadowTexCoord)
{
	...
	vShadowTexCoord.x *= m_fShadowPartitionSize;// precomputed (float) iCascadeIndex/(float)CASCADE_COUNT_FLAG
...
void CalculateBlendAmountForInterval ( in int iCurrentCascadeIndex, 
                                       in float fPixelDepthInWorldView, 
                                       ...
                                       ) 
{
	...
	float diffDepthInWorldView = fPixelDepthInWorldView - m_fCascadeFrustumsEyeSpaceDepthsFloat4[ fBlendIntervalbelowIndex ].x;
    ...
    fCurrentPixelsBlendBandLocation = diffDepthInWorldView / fBlendInterval;
...
float4 PSMain( VS_OUTPUT Input ) : SV_TARGET
{
    ...
    float4 vShadowMap_TargetTextureCoord = 0.0f;
    float4 vShadowMap_TargetTextureCoord_blend = 0.0f;
    ...
    float4 vShadowMapInTextureCoordViewSpace = Input.vTexInShadowView;
    if( SELECT_CASCADE_BY_INTERVAL_FLAG ) 
    {
        iCurrentCascadeIndex = 0;
        if ( CASCADE_COUNT_FLAG > 1 ) 
        {
            float4 vCurrentPixelDepth = Input.vDepthInWorldView;
    ...
    if ( !SELECT_CASCADE_BY_INTERVAL_FLAG ) 
    {
        ...
        if ( CASCADE_COUNT_FLAG  == 1 ) 
        {
            vShadowMap_TargetTextureCoord = vShadowMapInTextureCoordViewSpace * m_vCascadeScale[0];
            vShadowMap_TargetTextureCoord += m_vCascadeOffset[0];
        }
        if ( CASCADE_COUNT_FLAG > 1 ) {
            for( int iCascadeIndex = 0; iCascadeIndex < CASCADE_COUNT_FLAG && iCascadeFound == 0; ++iCascadeIndex ) 
            {
                vShadowMap_TargetTextureCoord = vShadowMapInTextureCoordViewSpace * m_vCascadeScale[iCascadeIndex];
                vShadowMap_TargetTextureCoord += m_vCascadeOffset[iCascadeIndex];

                if ( min( vShadowMap_TargetTextureCoord.x, vShadowMap_TargetTextureCoord.y ) > m_fMinBorderPadding
                  && max( vShadowMap_TargetTextureCoord.x, vShadowMap_TargetTextureCoord.y ) < m_fMaxBorderPadding )
                { 
    ...
    if( SELECT_CASCADE_BY_INTERVAL_FLAG ) 
    {
        if( BLEND_BETWEEN_CASCADE_LAYERS_FLAG && CASCADE_COUNT_FLAG > 1  ) 
         {
            CalculateBlendAmountForInterval ( iCurrentCascadeIndex, fCurrentPixelDepthInWorldView, 
    ...
    else 
    {
        ...
        if( BLEND_BETWEEN_CASCADE_LAYERS_FLAG ) 
        {
            CalculateBlendAmountForMap ( vShadowMap_TargetTextureCoord, 
                fCurrentPixelsBlendBandLocation, fBlendBetweenCascadesAmount );
        }   
    }
    ...
    if( USE_DERIVATIVES_FOR_DEPTH_OFFSET_FLAG ) 
    {
        vShadowMapTextureCoordDDX = ddx( vShadowMapInTextureCoordViewSpace );
        vShadowMapTextureCoordDDY = ddy( vShadowMapInTextureCoordViewSpace );    
	 ...
	 if( SELECT_CASCADE_BY_INTERVAL_FLAG ) 
    {
		TranformShadowToTexture3D(vShadowMapInTextureCoordViewSpace,iCurrentCascadeIndex,vShadowMap_TargetTextureCoord);
    }

	 TransformLogicU_ToNativeU(iCurrentCascadeIndex,vShadowMap_TargetTextureCoord);
    ...
    CalculatePCFPercentLit ( vShadowMap_TargetTextureCoord, fRightTextDepthWeight, 
    ...
    if( BLEND_BETWEEN_CASCADE_LAYERS_FLAG && CASCADE_COUNT_FLAG > 1 ) 
    {
        ...
        {  // the current pixel is within the blend band.
			...
			TranformShadowToTexture3D(vShadowMapInTextureCoordViewSpace,iNextCascadeIndex,vShadowMap_TargetTextureCoord_blend);

			TransformLogicU_ToNativeU(iNextCascadeIndex,vShadowMap_TargetTextureCoord_blend);
        ...
        // We repeat the calcuation for the next cascade layer, when blending between maps.
            ...
            {  // the current pixel is within the blend band.
                ...
                CalculatePCFPercentLit ( vShadowMap_TargetTextureCoord_blend, fRightTextDepthWeight_blend, 
